...
class TestConnectionPool:
    ...
    def test_connection_reuse(self, connection_pool):
        ...
        assert connection_pool._created_connections <= initial_count + connection_pool.pool_size
    ...
    def test_pool_stress_test(self, temp_db_path):
        ...
        for worker_id, results in worker_results.items():
            assert len(results) == 50, f"Worker {worker_id} didn't complete all iterations"
            # Check that no errors occurred
            error_count = sum(1 for r in results if isinstance(r, str) and r.startswith("ERROR:"))
            assert error_count == 0, f"Worker {worker_id} had errors: {[r for r in results if isinstance(r, str)]}"
    ...
    def test_repository_concurrent_operations(self, repository):
        ...
        def worker(worker_id):
            ...
            async def async_work():
                # Add a user
                user = await repository.add_user(f"user_{worker_id}", {"worker": worker_id})
        ...
        for worker_id, message_count in results:
            assert message_count == 5, f"Worker {worker_id} didn't complete all operations"
    
    def test_repository_cleanup(self, config):
        """Test that repository properly cleans up connections."""
        repo = sqlite_ChatHistoryRepository(config)
        initial_connections = repo.pool._created_connections
