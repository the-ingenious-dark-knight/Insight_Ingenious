...
class MockSQLRepository(BaseSQLRepository):
    ...
    def _execute_sql(self, sql: str, params: List[Any] = None, expect_results: bool = True) -> Any:
        ...
        query_key = sql.strip().split()[0].upper()  # Get first word (INSERT, SELECT, etc.)
...
class TestBaseSQLRepository:
    ...
    async def test_get_user_new(self, mock_repository):
        ...
        insert_queries = [q for q in mock_repository.executed_queries if "INSERT INTO users" in q['sql']]
    ...
    def test_row_to_message_tuple_format(self, mock_repository):
        ...
        row_tuple = (
            'user123', 'thread456', 'msg789', True, datetime.now(),
            'user', 'Test content', None, None, None, None
        )
    ...
    def test_row_to_user_tuple_format(self, mock_repository):
        ...
        row_tuple = (
            test_id, 'test_user', '{"role": "admin"}', '2023-01-01T00:00:00.000000+00:00'
        )
