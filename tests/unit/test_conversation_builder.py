...
class TestSyncPromptTemplates:
    ...
    async def test_sync_prompt_templates_non_local_storage_no_files(self):
        ...
        with patch('ingenious.utils.conversation_builder.FileStorage', return_value=mock_fs):
            with patch('ingenious.utils.conversation_builder.logger') as mock_logger:
                ...
                assert not any("Downloading template" in str(call) for call in mock_logger.debug.call_args_list)
    ...
    async def test_sync_prompt_templates_non_local_storage_with_files(self):
        ...
        mock_fs.read_file = AsyncMock(side_effect=[
            "Template 1 content",
            "Template 2 content"
        ])
        ...
        download_calls = [call for call in mock_logger.debug.call_args_list 
                         if "Downloading template" in str(call)]
        assert len(download_calls) == 2
        
        # Should have logged save activities
        save_calls = [call for call in mock_logger.debug.call_args_list 
    ...
    async def test_sync_prompt_templates_file_path_extraction(self):
        ...
        mock_fs = Mock()
        mock_fs.list_files = AsyncMock(return_value=[
            "deep/nested/path/template.jinja"
        ])
        ...
        mock_fs.read_file.assert_called_once_with(
            file_name="template.jinja", 
            file_path="prompts/v1.0"
        )