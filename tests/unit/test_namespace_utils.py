...
class TestNamespaceUtils:
    ...
    @patch('ingenious.utils.namespace_utils.import_module_with_fallback')
    def test_import_class_with_fallback_module_not_found(self, mock_import_module_fallback):
        """Test class import with fallback when module not found"""
        mock_import_module_fallback.side_effect = ModuleNotFoundError("Module not found")
        
        with pytest.raises(ValueError, match="Module test.module not found in any namespace"):
            import_class_with_fallback("test.module", "TestClass")

    @patch('ingenious.utils.namespace_utils.import_module_with_fallback')
    def test_import_class_with_fallback_attribute_error(self, mock_import_module_fallback):
        """Test class import with fallback when class not found"""
        mock_module = Mock()
        del mock_module.TestClass  # Remove attribute
        mock_import_module_fallback.return_value = mock_module
        
        with pytest.raises(ValueError, match="Class TestClass not found in module test.module"):
            import_class_with_fallback("test.module", "TestClass")
    ...
    @patch('ingenious.utils.namespace_utils.get_dir_roots')
    def test_get_file_from_namespace_with_fallback_file_not_found(self, mock_get_dir_roots):
    ...
    def test_import_module_with_fallback_success(self, mock_find_spec, mock_import_module, mock_get_namespaces):
