...
from ingenious.api.routes.conversation import router, get_conversation
...
class TestConversationAPI:
    ...
    async def test_get_conversation_success(self):
        ...
        mock_messages = [
            ...
            Message(role="assistant", content="Hi there!", user_id="user1", thread_id="thread_123")
        ]
    ...
    async def test_get_conversation_with_various_thread_ids(self):
        ...
        mock_repository = Mock()
        mock_messages = [Message(role="user", content="Test", user_id="user1", thread_id="thread_test")]
        mock_repository.get_thread_messages = AsyncMock(return_value=mock_messages)
