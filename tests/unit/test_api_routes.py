...
class TestChatRoutes:
    ...
    async def test_chat_endpoint_success(self):
        ...
        chat_request = ChatRequest(
            ...
            conversation_flow="test_workflow"
        )
    ...
    async def test_chat_endpoint_content_filter_error(self):
        ...
        mock_chat_service = Mock()
        mock_chat_service.get_chat_response = AsyncMock(side_effect=ContentFilterError("Content filtered"))
        
        chat_request = ChatRequest(
            user_prompt="Inappropriate message",
            conversation_flow="test_workflow"
        )
    ...
    async def test_chat_endpoint_token_limit_error(self):
        ...
        mock_chat_service = Mock()
        mock_chat_service.get_chat_response = AsyncMock(side_effect=TokenLimitExceededError("Token limit exceeded"))
        
        chat_request = ChatRequest(
            user_prompt="Very long message",
            conversation_flow="test_workflow"
        )
    ...
    async def test_chat_endpoint_generic_exception(self):
        ...
        mock_chat_service = Mock()
        mock_chat_service.get_chat_response = AsyncMock(side_effect=Exception("Generic error"))
...
class TestMessageFeedbackRoutes:
    ...
    async def test_submit_message_feedback_success(self):
        ...
        result = await submit_message_feedback("test_message_id", feedback, mock_service)
        
        assert result == mock_response
        mock_service.update_message_feedback.assert_called_once_with("test_message_id", feedback)
    
    @pytest.mark.asyncio
    async def test_submit_message_feedback_failure(self):
        """Test message feedback submission failure."""
        mock_service = Mock()
        mock_service.update_message_feedback = AsyncMock(side_effect=Exception("Database error"))
    ...
    async def test_submit_message_feedback_invalid_message_id(self):
        ...
        mock_service = Mock()
        mock_service.update_message_feedback = AsyncMock(side_effect=ValueError("Message not found"))
...
class TestConversationRoutes:
    ...
    async def test_get_conversation_repository_error(self):
        ...
        mock_repo = Mock()
        mock_repo.get_thread_messages = AsyncMock(side_effect=Exception("Database error"))
...
class TestDiagnosticRoutes:
    ...
    async def test_health_check_success(self):
        ...
        with patch('ingenious.api.routes.diagnostic.datetime') as mock_datetime, \
             patch('ingenious.api.routes.diagnostic.igen_deps.get_config') as mock_get_config, \
             patch('ingenious.api.routes.diagnostic.igen_deps.get_profile') as mock_get_profile:
    ...
    async def test_system_info_success(self):
        ...
        with patch('ingenious.api.routes.diagnostic.igen_deps.get_config') as mock_get_config:
            mock_get_config.return_value = Mock()
