...
class TestSyncPromptTemplates:
    ...
    async def test_sync_prompt_templates_azure_storage(self, mock_file_open, mock_mkdir, mock_file_storage):
        ...
        mock_fs_instance.list_files = AsyncMock(return_value=[
            "prompts/v1/template1.jinja",
            "prompts/v1/template2.jinja",
            "prompts/v1/not_jinja.txt"
        ])
        mock_fs_instance.read_file = AsyncMock(side_effect=[
            "Template 1 content",
            "Template 2 content"
        ])
    ...
    @patch('pathlib.Path.mkdir')
    @patch('builtins.open', new_callable=mock_open)
    async def test_sync_prompt_templates_no_jinja_files(self, mock_file_open, mock_mkdir, mock_file_storage):
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('ingenious.utils.conversation_builder.logger')
    async def test_sync_prompt_templates_with_logging(self, mock_logger, mock_file_open, mock_mkdir, mock_file_storage):
