...
class TestBasicDependencies:
    ...
    def test_get_chat_service(self):
        ...
        mock_container.chat_service_factory.assert_called_once_with(
            conversation_flow=""
        )
        assert result is mock_service
...
class TestSecurityService:
    ...
    @patch("ingenious.services.dependencies.get_username_from_token")
    def test_get_security_service_jwt_token_invalid_fallback_to_basic(
        ...
        result = get_security_service(
            token=mock_token, credentials=credentials, config=self.mock_config
        )
...
class TestSecurityServiceOptional:
    ...
    def test_get_security_service_optional_valid_credentials(self):
        ...
        result = get_security_service_optional(
            credentials=credentials, config=self.mock_config
        )
...
class TestAuthUser:
    ...
    def test_get_conditional_security(self):
        ...
        with patch(
            "ingenious.services.dependencies.get_auth_user", return_value="test_user"
        ) as mock_get_auth:
...
class TestSyncTemplates:
    ...
    def test_sync_templates_non_local_storage(
        self, mock_join, mock_getcwd, mock_fs_class, mock_asyncio_run
    ):
