...
class TestOpenAIService:
    ...
    def test_init_with_azure_config(self):
        ...
        with patch('ingenious.external_services.openai_service.AzureOpenAI') as mock_azure:
            mock_client = Mock()
    ...
    async def test_generate_response_content_filter_error(self):
        ...
        with patch('ingenious.external_services.openai_service.AzureOpenAI') as mock_azure:
            ...
            mock_error = BadRequestError("Content filter error", response=Mock(), body={
                "code": "content_filter",
    ...
    async def test_generate_response_token_limit_error(self):
        ...
        with patch('ingenious.external_services.openai_service.AzureOpenAI') as mock_azure:
            ...
            mock_error = BadRequestError(token_error_msg, response=Mock(), body={"message": token_error_msg})
            mock_client.chat.completions.create.side_effect = mock_error
            mock_azure.return_value = mock_client
    ...
    async def test_generate_response_generic_error(self):
        ...
        with patch('ingenious.external_services.openai_service.AzureOpenAI') as mock_azure:
            mock_client = Mock()
            from openai import BadRequestError
            mock_error = BadRequestError("An unexpected error occurred", response=Mock(), body={"message": "An unexpected error occurred"})
            mock_client.chat.completions.create.side_effect = mock_error
            mock_azure.return_value = mock_client
