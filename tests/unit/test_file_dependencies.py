...
class TestFileDependencies:
    ...
    def test_get_file_storage_data_function_docstring(self):
        ...
        assert (
            "Get file storage for data from container" in get_file_storage_data.__doc__
        )
    ...
    def test_get_file_storage_revisions_function_docstring(self):
        ...
        assert (
            ...
            in get_file_storage_revisions.__doc__
        )
    ...
    def test_sync_templates_with_non_local_storage(
        self, mock_join, mock_getcwd, mock_file_storage_class, mock_asyncio_run
    ):
    ...
    def test_sync_templates_file_operations(
        self,
        mock_file_open,
        mock_join,
        mock_getcwd,
        ...
        async def mock_sync_files():
            mock_fs_instance.list_files = AsyncMock(
                return_value=["template1.jinja", "template2.jinja"]
            )
            mock_fs_instance.read_file = AsyncMock(side_effect=["content1", "content2"])

            template_files = await mock_fs_instance.list_files(
                file_path="/test/dir/ingenious/templates"
            )
    ...
    def test_dependency_injection_decorators(self):
        ...
        assert hasattr(get_file_storage_data, "__wrapped__") or hasattr(
            get_file_storage_data, "_injected"
        )

        # get_file_storage_revisions should have @inject decorator
        assert hasattr(get_file_storage_revisions, "__wrapped__") or hasattr(
            get_file_storage_revisions, "_injected"
        )
