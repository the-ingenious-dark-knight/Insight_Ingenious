...
class DocumentationValidator:
    ...
    def validate_cli_commands(self) -> bool:
        ...
        documented_commands = {
            ...
            'help', 'status', 'version', 'document-processing', 'dataprep'
        }
    ...
    def validate_workflows(self) -> bool:
        ...
        flows_path = self.project_root / 'ingenious' / 'services' / 'chat_services' / 'multi_agent' / 'conversation_flows'
        for workflow, folder in core_workflows.items():
            workflow_path = flows_path / folder
            if not workflow_path.exists():
                self.errors.append(f"Core workflow {workflow} not found at {workflow_path}")
                
        # Check template workflows
        template_path = self.project_root / 'ingenious' / 'ingenious_extensions_template' / 'services' / 'chat_services' / 'multi_agent' / 'conversation_flows'
        for workflow, folder in template_workflows.items():
            workflow_path = template_path / folder
            if not workflow_path.exists():
                self.warnings.append(f"Template workflow {workflow} not found at {workflow_path}")
    ...
    def validate_dependencies(self) -> bool:
        ...
        for dep in required_deps:
            if dep not in content:
                self.warnings.append(f"Expected dependency {dep} not found in pyproject.toml")
