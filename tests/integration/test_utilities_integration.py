...
from ingenious.utils.env_substitution import substitute_env_vars, load_yaml_with_env_substitution
...
class TestNamespaceUtilsIntegration:
    ...
    def test_module_import_fallback_chain(self, mock_find_spec, mock_import_module, mock_get_namespaces):
        ...
        mock_find_spec.side_effect = [None, Mock(), None]  # Only second namespace has the module
        mock_module = Mock()
        ...
        with patch('ingenious.utils.namespace_utils.Path.cwd') as mock_cwd, \
            ...
            mock_import_module.assert_called_with("ingenious.ingenious_extensions_template.test.module")
    ...
    def test_file_retrieval_fallback_chain(self, mock_get_dir_roots):
        ...
        with patch('ingenious.utils.namespace_utils.os.path.exists', side_effect=mock_exists), \
    ...
    def test_path_retrieval_fallback_chain(self, mock_get_dir_roots):
        ...
        def mock_exists(path):
            return str(path) == str(dir2 / "test/path") or str(path) == str(dir3 / "test/path")
...
"""
        ...
        try:
            with patch.dict(os.environ, {
                ...
                assert 'true' in result  # Both FEATURES_ENABLED default and DEBUG_MODE env
        ...
        for content, env_vars, expected, not_expected in test_cases:
            with patch.dict(os.environ, env_vars, clear=True):
                ...
                for expected_item in expected:
                    assert expected_item in result, f"Expected '{expected_item}' in result for content: {content}"
                
                for not_expected_item in not_expected:
                    assert not_expected_item not in result, f"Did not expect '{not_expected_item}' in result for content: {content}"