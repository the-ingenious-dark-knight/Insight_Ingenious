...
from ingenious.db.query_builder import QueryBuilder, SQLiteDialect, AzureSQLDialect
...
class TestSQLiteIntegration:
    ...
    async def test_add_and_get_message(self, sqlite_repository):
        ...
        # Retrieve message
        retrieved_message = await sqlite_repository.get_message(message_id, message.thread_id)
    ...
    async def test_update_message_feedback(self, sqlite_repository):
        ...
        # Update feedback
        await sqlite_repository.update_message_feedback(message_id, message.thread_id, True)
        ...
        # Update again
        await sqlite_repository.update_message_feedback(message_id, message.thread_id, False)
    ...
    def test_query_builder_generates_working_sql(self, temp_db_path):
        ...
        cursor.execute(insert_query, [user_id, "test@example.com", "{}", datetime.now().isoformat()])
...
class TestDatabaseAgnosticBehavior:
    ...
    def test_dialect_compatibility(self, temp_db_path):
        ...
        test_data = [
            "user_123", "thread_456", "msg_789", True, 
            datetime.now().isoformat(), "user", "Hello",
            None, None, None, None
        ]
    ...
    def test_connection_pool_stress(self, temp_db_path):
        ...
        def use_connection(iteration):
            try:
                with pool.get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute("INSERT INTO test_table (value) VALUES (?)", [f"value_{iteration}"])
        ...
        for i in range(10):  # More threads than pool size
            thread = threading.Thread(target=lambda i=i: results.append(use_connection(i)))
        ...
        assert error_count == 0, f"Got {error_count} errors: {[r for r in results if isinstance(r, str)]}"
