...
class TestAPIDependencyInjection:
    ...
    def mock_chat_service(self):
        ...
        service.get_chat_response.return_value = Mock(
            message="Test response", thread_id="test-thread", message_id="test-message"
        )
        return service
    ...
    def client(
        ...
        with patch("ingenious.services.container.init_container") as mock_init:
            ...
            mock_container.chat_history_repository.return_value = (
                mock_chat_history_repository
            )
            mock_container.message_feedback_service.return_value = (
                mock_message_feedback_service
            )
    ...
    def test_conversation_endpoint_dependency_injection(
        self, client, mock_chat_history_repository
    ):
        """Test that conversation endpoint receives injected repository correctly."""
        with patch(
            "ingenious.services.dependencies.get_chat_history_repository"
        ) as mock_get_repo:
    ...
    def test_message_feedback_endpoint_dependency_injection(
        self, client, mock_message_feedback_service
    ):
        """Test that message feedback endpoint receives injected service correctly."""
        with patch(
            "ingenious.services.dependencies.get_message_feedback_service"
        ) as mock_get_service:
    ...
    def test_dependency_override_in_testing(self):
        ...
        from ingenious.services.container import get_container, override_for_testing
    ...
    def test_complex_dependency_graph(self):
        ...
        with patch.object(container, "config") as mock_config_provider:
            with patch.object(
                ...
                with patch(
                    "ingenious.services.container._create_chat_service"
                ) as mock_create:
                    mock_service = Mock()
