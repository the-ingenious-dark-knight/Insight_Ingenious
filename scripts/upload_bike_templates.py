...
def read_local_template(filename):
    ...
    template_path = Path("ingenious/ingenious_extensions_template/templates/prompts") / filename
...
def main():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"✅ Verified: {data['count']} templates now available in Azure Blob Storage")
