...
def flatten_dict(data: Dict[str, Any], parent_key: str = "", separator: str = "__") -> Dict[str, Any]:
    """
        ...
        if isinstance(value, dict):
            ...
            for i, item in enumerate(value):
                if isinstance(item, dict):
                    items.extend(flatten_dict(item, f"{new_key}__{i}", separator).items())
                else:
...
def write_env_file(env_vars: Dict[str, str], output_file: Path, include_comments: bool = True) -> None:
    """
    ...
    with open(output_file, 'w') as f:
        if include_comments:
            ...
            f.write("# See: https://docs.pydantic.dev/latest/concepts/pydantic_settings/\n\n")
        ...
        for key, value in sorted(env_vars.items()):
            ...
            elif any(service in key for service in ["CHAINLIT", "PROMPT_TUNER", "TOOL_SERVICE", "CHAT_SERVICE"]):
...
Examples:
  ...
  %(prog)s --yaml-file config.yml --validate-only
    ...
    print(f"✓ Converted {len(env_vars)} configuration settings to environment variables")
    ...
    if args.validate:
        ...
        print(f"  python -c \"from ingenious.config.settings import IngeniousSettings; IngeniousSettings(_env_file='{args.output}')\"")
    
    print(f"\nNext steps:")
    print(f"1. Review the generated file: {args.output}")
    print(f"2. Copy settings to your .env file")
    print(f"3. Set any placeholder values to real credentials")
    print(f"4. Test with: uv run python -c \"from ingenious.config.config import get_config; get_config()\"")
