...
class ChatHistorySettings(BaseModel):
    ...
    memory_path: str = Field(
        "./tmp",
        description="Path for memory storage and temporary files"
    )
...
class ModelSettings(BaseModel):
    ...
    api_type: str = Field(
        "rest",
        description="API type: 'rest' for HTTP APIs"
    )
...
class ChainlitSettings(BaseModel):
    ...
    class AuthenticationSettings(BaseModel):
        ...
        enable: bool = Field(
            False,
            description="Enable authentication for Chainlit"
        )
...
class PromptTunerSettings(BaseModel):
    ...
    mode: str = Field(
        "fast_api",
        description="Mode for prompt tuner: 'fast_api' for web interface"
    )
    enable: bool = Field(
        True,
        description="Enable prompt tuner interface"
    )
...
class ToolServiceSettings(BaseModel):
    ...
    enable: bool = Field(
        False,
        description="Enable external tool service integrations"
    )
...
class AzureSqlSettings(BaseModel):
    ...
    table_name: str = Field(
        "",
        description="Default table name for operations"
    )
    database_connection_string: str = Field(
        "",
...
class WebAuthenticationSettings(BaseModel):
    ...
    type: str = Field(
        "basic",
        description="Authentication type: 'basic' for HTTP basic auth"
    )
...
class WebSettings(BaseModel):
    ...
    type: str = Field(
        "fastapi",
        description="Web framework type: 'fastapi' for FastAPI"
    )
    asynchronous: bool = Field(
        False,
        description="Enable asynchronous response handling"
    )
...
class LocalSqlSettings(BaseModel):
    ...
    sample_database_name: str = Field(
        "sample_sql_db",
        description="Name for the sample database"
    )
...
class FileStorageContainerSettings(BaseModel):
    ...
    enable: bool = Field(
        True,
        description="Enable this storage container"
    )
    ...
    token: str = Field(
        "",
        description="Azure access token (for Azure storage only)"
    )
...
class ReceiverSettings(BaseModel):
    ...
    enable: bool = Field(
        False,
        description="Enable external event receiver"
    )
    api_url: str = Field(
        "",
        description="URL for receiving external events"
    )
    api_key: str = Field(
        "DevApiKey",
        description="API key for authenticating external events"
    )
...
class IngeniousSettings(BaseSettings):
    ...
    profile: str = Field(
        "default",
        description="Profile name to use for environment-specific settings"
    )
    ...
    azure_sql_services: Optional[AzureSqlSettings] = Field(
        default=None,
        description="Azure SQL service configuration (optional)"
    )
    ...
    def validate_configuration(self) -> None:
        ...
        if self.web_configuration.authentication.enable and not self.web_configuration.authentication.password:
            errors.append(
    ...
    def create_minimal_config(cls) -> "IngeniousSettings":
        ...
        return cls(
            ...
            web_configuration=WebSettings(
                port=8000,
                authentication=WebAuthenticationSettings(enable=False)
            )
        )