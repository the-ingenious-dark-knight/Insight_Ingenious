...
class Config(config_models.Config):
    ...
    def from_yaml_str(config_yml):
        ...
        try:
            config_ns = config_ns_models.Config.model_validate_json(
                json_data
            )
        ...
        profile_object: profile_models.Profile = \
            profile_data.get_profile_by_name(
                config_ns.profile
            )
        if profile_object is None:
            raise ValueError(
                f"Profile {config_ns.profile} not found in profiles.yml"
            )

        config: config_models.Config = config_models.Config(
            config_ns, profile_object
        )
...
def get_config(config_path=None) -> config_models.Config:
    ...
    if path.exists:
        if Path(config_path).is_file():
            ...
            logger.debug(f"Config file at {config_path} is not a file. Falling back to key vault")
            try:
