...
class ImportError(Exception):
    ...
    def __init__(self, message: str, module_name: str = None, class_name: str = None, 
                 attempted_paths: List[str] = None, original_error: Exception = None):
...
class SafeImporter:
    ...
    def _get_namespace_roots(self) -> List[Path]:
        ...
        return [
            ...
            Path(importlib.util.find_spec("ingenious").origin).parent if importlib.util.find_spec("ingenious") else None
        ]
    ...
    def _validate_class(self, cls: Type, expected_protocol: Optional[Type] = None, 
                       expected_methods: Optional[List[str]] = None) -> None:
    ...
    def import_module_with_fallback(self, module_name: str, expected_attrs: Optional[List[str]] = None,
                                   use_cache: bool = True) -> Any:
        """
        ...
        for namespace in self._namespaces:
            ...
            try:
                ...
                if self._find_module_spec(full_module_name):
                    module = self.import_module(full_module_name, expected_attrs=expected_attrs, use_cache=False)
    ...
    def import_class_with_fallback(self, module_name: str, class_name: str,
                                  expected_protocol: Optional[Type] = None,
                                  expected_methods: Optional[List[str]] = None,
                                  use_cache: bool = True) -> Type:
        """
    ...
    def clear_cache(self, pattern: Optional[str] = None) -> None:
        ...
        logger.debug(f"Cleared import caches" + (f" matching pattern: {pattern}" if pattern else ""))
...
def import_class_safely(module_name: str, class_name: str, 
                       expected_methods: Optional[List[str]] = None) -> Type:
...
def import_module_with_fallback(module_name: str, expected_attrs: Optional[List[str]] = None) -> Any:
    """Import a module with namespace fallback support."""
...
def import_class_with_fallback(module_name: str, class_name: str, 
                              expected_methods: Optional[List[str]] = None) -> Type:
    """Import a class with namespace fallback support."""
    return _global_importer.import_class_with_fallback(module_name, class_name, expected_methods=expected_methods)
