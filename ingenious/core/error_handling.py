...
class RateLimitRecoveryStrategy(RecoveryStrategy):
    """Recovery strategy for rate limit errors with adaptive backoff."""
    ...
    def recover(
        ...
        **kwargs: Any,
        ...
        if retry_after:
            ...
            delay = min(
                self.base_delay * (2**self.consecutive_failures), self.max_delay
            )
            delay *= 0.5 + random.random() * 0.5  # Add jitter

            logger.info(
                "Rate limit hit, using exponential backoff",
                delay_seconds=delay,
...
class DeadlockRecoveryStrategy(RecoveryStrategy):
    """Recovery strategy for database deadlock errors."""
    ...
    def recover(
        ...
        **kwargs: Any,
        ...
        for attempt in range(self.max_retries):
            # Random delay to reduce likelihood of repeated deadlocks
            ...
            try:
                ...
                if attempt == self.max_retries - 1:
                    logger.error(
                        "Deadlock recovery failed after all retries",
                        final_attempt=True,
                        operation="deadlock_recovery",
                    )
                    raise exc
...
class CompositeRecoveryStrategy(RecoveryStrategy):
    ...
    def recover(self, error: IngeniousError, *args: Any, **kwargs: Any) -> Any:
        ...
        applicable_strategies = [s for s in self.strategies if s.can_recover(error)]

        if not applicable_strategies:
            raise error

        for i, strategy in enumerate(applicable_strategies):
            try:
                logger.info(
                    f"Attempting recovery with strategy {i + 1}",
                    strategy_type=strategy.__class__.__name__,
                    operation="composite_recovery",
                )
                return strategy.recover(error, *args, **kwargs)
                ...
                if i == len(applicable_strategies) - 1:
                    # Last strategy failed, raise original error
                    raise error
                continue
