...
app = Flask(
    __name__,
    static_folder="flask_site_assets",
    static_url_path="/static",
    template_folder="flask_site_pages",
)
...
def create_revision():
    if request.method == "POST":
        ...
        if asyncio.run(
            ...
            revisions_str = asyncio.run(
                fs.read_file(file_name="revisions", file_path=str(REVISIONS_FOLDER))
            )
            revisions = yaml.safe_load(revisions_str)
        else:
            revisions = []
...
def edit_markdown():
    try:
        files_raw = asyncio.run(
            fs.list_files(file_path=f"prompts/{get_selected_revision_direct_call()}")
        )
        files = sorted(
...
def get_payload():
    ...
    return asyncio.run(
        rp1.render_dashboard(ball_identifier, fs, output_dir, event_type)
    )
